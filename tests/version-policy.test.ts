...
// version a caret range is exactly a minor line (`^0.9.2` == `>=0.9.2 <0.10.0`),
...
/** `0.9.2` -> `0.9`. Returns null for anything that is not plain semver. */
...
/** `^0.9.2` -> `0.9.2`. The range floor is the minimum compatible CLI version. */
...
describe("version policy", () => {
  test("the akm range is a caret range, so a minor line is what it pins", () => {
    // The whole policy rests on `^0.9.2` meaning ">=0.9.2 <0.10.0". If the range
  ...
  test("every stamped version is plain semver", () => {
    ...
    // `0.9.2.20260811.1` reads as a reasonable way to encode a dated build, but
