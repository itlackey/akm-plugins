...
// Session-end / Stop / session.idle / session.compacted / session.deleted
// lifecycle handler.
//
// Until 0.8.0-rcN the OpenCode plugin flushed an in-memory session buffer into
// a durable `memory:opencode-session-YYYYMMDD-<sid>` (or
// `memory:opencode-checkpoint-*` for mid-session checkpoints and proposal-prep
// flows) on every Stop / session.idle / session.compacted / session.deleted /
// pre-improve / pre-propose / pre-evolve event. That writer produced
// low-signal `akm_memory_kind: session_checkpoint` files that polluted the
// proposal queue (often with a stacked-frontmatter bug), so it was removed.
//
// The replacement is the new `akm extract --type opencode --session-id <id>`
// CLI (akm core release/0.8.0), which reads native OpenCode session JSON +
// per-message JSON files through the SessionLogHarness registry and produces
    ...
    "chat.message": async (input, output) => {
      try {
        ...
        if (
          ...
          // Pull recent refs from the shared retrospective state (populated
          // by `noteRecentRefs` in `tool.execute.after` / `chat.message`).
          // The per-session checkpoint buffer that used to back this signal
          // was removed alongside the session-checkpoint memory writer.
          const recentRefs = (retrospectiveState.get(input.sessionID)?.recentRefs ?? [])
            .filter((ref) => !ref.startsWith("memory:") && !ref.startsWith("vault:"))
            .slice(-3)
    ...
    "tool.execute.after": async (input, output) => {
      try {
        ...
        if (allRefs.length > 0) {
          ...
          // Keep `retrospectiveState.recentRefs` warm for the chat.message
          // retrospective-positive feedback path: every observed ref counts as
          // a recent-touch signal, even before tool-success classification.
          noteRecentRefs(input.sessionID, allRefs)
        }
    ...
    akm_memory: tool({
      ...
      async execute(input, context) {
        ...
        if (input.action === "checkpoint") {
          // The session-checkpoint memory writer was removed in 0.8.0 — see
          // the `handleSessionEnd` comment block. Surface a clear deprecation
          // pointer instead of silently no-oping so legacy callers can migrate.
          return JSON.stringify({
            ok: false,
            error: "akm_memory action='checkpoint' was removed in 0.8.0. The plugin no longer writes per-session memory:opencode-session-* / memory:opencode-checkpoint-* files. Use `akm extract --type opencode --session-id <sid>` to derive proposal candidates from the native OpenCode session JSON instead.",
          })
        }
