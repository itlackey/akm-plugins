...
const sessionCurrentModel = new Map<string, { providerID: string; modelID: string }>()
...
// Returns true when akm has a top-level `llm` config block set, false when it
// does not. Returns true on error (fail-open) to avoid incorrectly installing
// the proxy when akm is unavailable. Takes an optional cache pointer so callers
// can memoize the result for the process lifetime without a module-level var.
function hasAkmLlmConfigured(cache?: { value: boolean | undefined }): boolean {
  if (cache && cache.value !== undefined) return cache.value
  const result = runCliSyncRaw(["--format", "json", "-q", "config", "get", "llm"], 5_000)
  ...
  if (!result.ok) {
    ...
    if (!raw || raw === "null" || raw === "{}" || raw === "[]") {
      ...
      if (parsed === null || parsed === undefined) {
        configured = false
      } else if (typeof parsed === "object" && !Array.isArray(parsed) && Object.keys(parsed as Record<string, unknown>).length === 0) {
        configured = false
      } else {
        configured = true
      }
  ...
  if (cache) cache.value = configured
  return configured
}

// Resolves the state directory for the OpenCode plugin, used to persist the
// proxy shim across shell.env calls. Mirrors the XDG_STATE_HOME convention.
function getPluginStateDir(): string {
  const xdgState = process.env.XDG_STATE_HOME
  const base = xdgState || path.join(process.env.HOME || process.env.USERPROFILE || "/tmp", ".local", "state")
  return process.env.AKM_PLUGIN_STATE_DIR || path.join(base, "akm-opencode")
}

// Builds the shim script content. The shim reads a JSON prompt from stdin and
// forwards it to the harness provider using API keys already in the environment
// (inherited from OpenCode). Provider credentials are never written to disk.
function buildLlmProxyShimContent(): string {
  return `#!/usr/bin/env sh
# AKM LLM proxy shim — generated by akm-opencode v${PLUGIN_VERSION}
# Lends the harness provider connection to akm when no akm.llm is configured.
# Contract: reads JSON {"prompt":"...","system":"...","model":"..."} from stdin,
#           writes completion text to stdout.
# Exit 0 on success; non-zero causes akm index passes to degrade to no-op.
# Provider credentials are never stored — the shim inherits them from the
...
// Writes the LLM proxy shim to the plugin state directory and returns its path.
// Returns null when the shim could not be written (the caller treats this as
// no-op rather than an error).
function installLlmProxyShim(stateDir: string): string | null {
  try {
    mkdirSync(stateDir, { recursive: true })
    const shimPath = path.join(stateDir, "akm-llm-proxy.sh")
    writeFileSync(shimPath, buildLlmProxyShimContent(), { mode: 0o755 })
    return shimPath
  } catch {
    return null
  }
...
export const AkmPlugin: Plugin = async ({ client, worktree, directory }) => {
  ...
  // Per-instance cache for the LLM config check so each plugin instance (test
  // run, reloaded session) gets a fresh lookup rather than sharing stale state.
  const llmConfiguredCache: { value: boolean | undefined } = { value: undefined }
  ...
  return {
      ...
      try {
        ...
        if (type === "session.created" || type === "session.updated") {
          ...
          if (type === "session.deleted") {
            ...
            sessionCurrentModel.delete(sid)
          }
        }
      } catch {
    ...
    "experimental.chat.system.transform": async (
      input: { sessionID?: string; session_id?: string; model?: { providerID?: string; modelID?: string } } | undefined,
      output: { system?: string[] } | undefined,
    ) => {
      try {
        if (!output || !Array.isArray(output.system)) return
        const sid = extractSessionIdFromEvent(input) ?? ""
        // Capture the session model so shell.env can hint the proxy shim.
        if (sid && input?.model?.providerID && input?.model?.modelID) {
          sessionCurrentModel.set(sid, { providerID: input.model.providerID, modelID: input.model.modelID })
        }
    ...
    "shell.env": async (input, output) => {
      try {
        ...
        // When akm has no llm block configured, lend the harness provider
        // connection by installing a proxy shim and advertising its path.
        // The shim inherits provider credentials from the environment (they
        // are never written to disk). AKM consults AKM_LLM_PROXY_CMD when its
        // own llm config is absent so index passes can still run.
        if (!hasAkmLlmConfigured(llmConfiguredCache)) {
          const stateDir = getPluginStateDir()
          const shimPath = installLlmProxyShim(stateDir)
          if (shimPath) {
            output.env.AKM_LLM_PROXY_CMD = shimPath
            // Hint the shim toward the current session model when known.
            const sid = typeof input?.sessionID === "string" ? input.sessionID : undefined
            const model = sid ? sessionCurrentModel.get(sid) : undefined
            if (model) output.env.AKM_LLM_PROXY_MODEL = model.modelID
          }
        }
      } catch {
        // Best-effort only.
