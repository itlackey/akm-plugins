...
The 0.9.14 `--pack` response already contains the selected local assets' full payloads within one shared token budget. Registry hits are deliberately omitted from packed output. If the packed array is empty, rerun `akm curate "$ARGUMENTS" --from all --limit 5 --format json -q` without `--pack` and report its refs or install guidance instead of inventing content. For each packed match, summarize:
- what the asset does
