...
| Session or subagent stops; before compaction | A structured `session_ended` event is recorded and `akm index` is run. Durable per-session memories are no longer auto-written — the pre-0.8.0 checkpoint writer was removed in favor of `akm extract --type claude-code --session-id <sid>`, which derives proposal candidates from the native Claude Code session JSONL when invoked explicitly. | Run `akm extract --type claude-code` (or `/akm-improve` against an existing ref) when you want to capture durable learnings, then review and accept the resulting proposals via `/akm-proposal accept <id>`. |
