...
Claude Code plugin for [AKM](https://github.com/itlackey/akm) `^0.9.14`. It provides an AKM skill, five slash commands, and lifecycle hooks that curate context and learn from concept usage.
...
The hooks require Bun 1.0 or newer on `PATH`. AKM must also be installed, available on `PATH`, and satisfy `^0.9.14`; the session-start hook reports a degraded status when either dependency is unavailable and does not install software automatically.
...
| `AKM_PACKAGE_REF` | `akm-cli@^0.9.14` | Package specification shown when AKM is unavailable. It is never installed automatically. |
...
| `AKM_CURATE_MIN_SCORE` | `0` (disabled) | Minimum per-item relevance score to keep a curated result. `0` preserves the long-standing behavior: whatever `akm curate` returns (up to `AKM_CURATE_LIMIT` results) is shown, unfiltered. Set it above `0` and two things change together: items below the floor are dropped — if none survive, no curated block is written at all, rather than showing weak results because there was nothing better — and the surviving items are reordered so locally authored assets (lesson, memory, knowledge, skill, command, agent, instruction, fact, workflow, task, env, secret) come before imported snapshots (website, wiki, or any other adapter-defined type). `akm curate`'s score scale is not documented as a stable contract (see akm's `STABILITY.md`), so there is no universally "right" value here — start from `curate --format json` output on your own stash and see what your strong hits score. |
| `AKM_CURATE_TYPE` | unset | Passthrough for `akm curate --type`. Narrows curation to a single exact asset type instead of competing against every type for the same `AKM_CURATE_LIMIT` slots. |
| `AKM_CONTEXT_BUDGET_CHARS` | `4000` | Maximum length of the text a hook injects into the model's context. Curated results are injected directly and trimmed to this budget; the full, untrimmed results are also written to a file under the plugin state dir for follow-up and debugging. The user-visible `systemMessage` channel is not subject to it. |
