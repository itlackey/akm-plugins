...
This document is the **canonical source of truth** for the `akm_help` tool (opencode
plugin) and the `/akm-help` slash command (Claude plugin). The table below is parsed
and pushed into the embedded copies by `scripts/generate-help-tables.mjs`:
...
Each consumer file marks the generated region with `BEGIN GENERATED` / `END GENERATED`
comments. Do not hand-edit the table inside those markers — edit this file instead, then
regenerate:

```sh
node scripts/generate-help-tables.mjs        # rewrite all three consumer copies
node scripts/generate-help-tables.mjs --check  # verify copies match this file; exits 1 and
                                                # names the drifted consumer/row otherwise
```

`tests/help-registry-parity.test.ts` shells out to `--check` so any row that drifts
between this file and its embedded copies fails `bun test tests/`; `--check` also runs
as its own step in `.github/workflows/tests.yml`.
